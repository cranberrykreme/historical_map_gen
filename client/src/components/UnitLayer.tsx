...
function UnitLayer({ units, onUnitDrag, onUnitMove, onUnitRotate, onUnitRotateCommit, onUnitScale, onUnitScaleCommit, onUnitSelect, setCursor, selectedUnitIds, scaleRef, isDraggingUnit, isShiftHeld }: UnitLayerProps) {
  ...
  const handleMouseDown = (e: React.MouseEvent, unitId: string) => {
    ...
    // If multiple units selected and clicking one of them, keep selection.
    if (selectedUnitIds.size > 1 && selectedUnitIds.has(unitId)) {
        // don't change selection
    } else {
        onUnitSelect(unitId, e.shiftKey);
    }
